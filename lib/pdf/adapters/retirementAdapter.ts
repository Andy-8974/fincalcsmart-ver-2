...
export function buildRetirementReportData(
  ...
  const p1 = input.hasGoal
    ? isOnTrack
      ...
      : `Your retirement plan projects ${fmt(input.projectedSavings)} by age ${input.retirementAge} — ${fmt(input.gapOrSurplus)} short of your ${fmt(input.retirementGoal)} goal (${Math.round(input.goalProgressPct)}% of goal reached). To close this gap, your estimated total monthly contribution would need to be approximately ${fmtx(input.requiredMonthly)}, which is about ${fmtx(input.additionalMonthlyNeeded)} more than your current ${fmtx(input.monthlyContribution)} monthly contribution. Retirement Readiness Score: ${input.readinessScore}/100 (${input.readinessLabel}).`
