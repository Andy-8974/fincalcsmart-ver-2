...
import {
  ...
  HelpCircle, Mail, PiggyBank, ShieldAlert, Sparkles, TrendingUp, Zap,
} from 'lucide-react';
...
function computeResults(form: FormState, freq: FreqKey, yearsInvested: number): TFSAResults | null {
  ...
  // Need at least some capital or contribution to show results
  ...
  const taxFreeGrowth      = Math.max(0, projectedValue - totalContributions);
  const growthPct          = totalContributions > 0 ? (taxFreeGrowth / totalContributions) * 100 : 0;

  // ── Room check (first-year plan) ──
  ...
  const growthLabel: TFSAResults['growthLabel'] =
    growthScore >= 80 ? 'Excellent' : growthScore >= 65 ? 'Good' : growthScore >= 45 ? 'Fair' : 'Poor';
  const growthStatus: TFSAResults['growthStatus'] =
  ...
  // ── Pill state ──
  const roomPillState: TFSAResults['roomPillState'] =
    availableRoom === 0 ? 'no-room-entered' :
    overRoom            ? 'over' :
    roomRemaining === 0 ? 'full' :
    'remaining';

  // ── Milestone projections for insight cards ──
  ...
  return {
    currentBalance, availableRoom, plannedOneTime, monthlyContribution, annualRate,
    freq, yearsInvested, effectiveMonthlyRate: r,
    effectivePrincipal, projectedValue, totalContributions, taxFreeGrowth, growthPct,
  ...
  async function handleDownloadPDF() {
    ...
    try {
      await buildCanadaRegisteredPDF({
        ...
        taxFreeGrowth:      results.taxFreeGrowth,
  ...
  return (
    <div className="flex flex-col gap-y-7 md:gap-y-8 min-w-0">
      ...
      <style>{`
        ...
        .btn-ai-cta-tfsa {
          animation: teal-glow-tfsa 2.8s ease-in-out infinite;
          transition: transform 180ms ease, box-shadow 180ms ease;
        }
        .btn-ai-cta-tfsa:hover {
          transform: translateY(-2px);
          box-shadow: 0 0 0 1px rgba(29,181,132,0.75), 0 0 32px rgba(29,181,132,0.38) !important;
          animation: none;
        }
        .tfsa-pill      { flex: 1; min-width: 0; text-align: center; }
        .tfsa-year-pill { flex: 1; min-width: 0; text-align: center; }
        .tfsa-pills     { display: flex; flex-wrap: wrap; gap: 6px; }
      `}</style>
          ...
          <div className="p-5 md:p-6">
            ...
            <div className="flex items-center gap-3 mb-4 md:mb-5">
              <div style={{
                ...
                <PiggyBank size={15} color="white" strokeWidth={2.2} aria-hidden />
              </div>
              <div>
                <p className="text-[10px] font-bold uppercase tracking-[0.12em] mb-0.5" style={{ color: '#1DB584' }}>
                  Canada · Tax-Free
                </p>
                <p className="text-base font-bold" style={{ color: '#0D1B2A' }}>TFSA Details</p>
              </div>
            </div>

            {/* ── Main 2-col grid: row1 Balance|One-Time, row2 Room|Rate ── */}
            <div className="grid grid-cols-2 gap-x-3 md:gap-x-5 mb-3">

              {/* Left: Balance + Available Room */}
              <div className="space-y-3 min-w-0">
                <div>
                  <label className={navyLabelCls} style={navyLabelStyle}>
                    Current TFSA Balance
                    <Tooltip text="The current value of your TFSA. This is your starting balance for the projection — not a contribution room figure." />
                  </label>
                  <NumericInput
                ...
                <div>
                  <label className={navyLabelCls} style={navyLabelStyle}>
                    Available Room
                    <Tooltip text="Your personal TFSA contribution room as shown in CRA My Account or by your financial institution. This calculator does not calculate or verify your exact room." />
                  </label>
                  <NumericInput
                    ...
                    inputClassName={inputClsCompact}
                  />
                  <p className="mt-0.5 text-[10px]" style={{ color: '#9BA8B5' }}>Verify with CRA My Account</p>
                </div>
              </div>

              {/* Right: One-Time Contribution + Expected Annual Return */}
              <div className="border-l pl-2 md:pl-5 space-y-3 min-w-0" style={{ borderColor: 'rgba(15,41,66,0.08)' }}>
                <div>
                  <label className={navyLabelCls} style={navyLabelStyle}>
                    One-Time Contribution
                    <Tooltip text="A lump sum you plan to contribute now. Checked against your entered available room together with your first year of monthly contributions." />
                  </label>
                  <NumericInput
                ...
                <div>
                  <label className={navyLabelCls} style={navyLabelStyle}>
                    Annual Return
                    <Tooltip text="Assumed annual growth rate for illustrations. Actual TFSA returns depend on your investments and are not guaranteed." />
                  </label>
                  <NumericInput
            ...
            {/* Monthly Contribution — left-column width, aligned with grid */}
            <div className="grid grid-cols-2 gap-x-3 md:gap-x-5 mb-4">
              <div className="min-w-0">
                <label className={navyLabelCls} style={navyLabelStyle}>
                  Monthly Contribution
                  <Tooltip text="An amount added to your TFSA each month. The first 12 months are included in the first-year contribution room check." />
                </label>
                <NumericInput
              ...
              <div /> {/* spacer keeps grid column width consistent */}
            </div>

            {/* ── Pill groups: full-width on mobile, 2-col on md+ ── */}
            <div
            ...
            >
              <div>
                ...
                <div className="tfsa-pills">
                  {FREQ_OPTIONS.map((f) => (
                    <button
                      key={f.key}
                      type="button"
                      className="tfsa-pill font-semibold text-xs py-1.5 rounded-lg transition-colors duration-150"
                ...
                <div className="tfsa-pills">
                  {YEAR_OPTIONS.map((yr) => (
                    <button
                      key={yr}
                      type="button"
                      className="tfsa-year-pill font-semibold text-xs py-1.5 rounded-lg transition-colors duration-150"
                      onClick={() => setYears(yr)}
                      style={
                        yearsInvested === yr
                          ? { background: '#1DB584', color: '#fff', border: '1.5px solid #1DB584' }
            ...
            {!results && (
              <div className="flex flex-1 items-center justify-center py-12">
                <p style={{ color: 'rgba(255,255,255,0.25)', fontSize: '14px', textAlign: 'center', lineHeight: 1.7 }}>
                  Enter a TFSA balance or contribution amount to see your projection.
                </p>
              </div>
            )}
              ...
              <div className="flex flex-col flex-1 gap-4">
                ...
                >
                  ...
                  <p style={{ color: 'rgba(255,255,255,0.38)', fontSize: '11px', marginTop: 4, fontWeight: 500 }}>
                    after {results.yearsInvested} years · tax-free growth illustration
                  </p>
                </div>
                ...
                <div className="space-y-0 px-1 divide-y" style={{ borderColor: 'rgba(255,255,255,0.07)' }}>
                  <div className="flex items-center justify-between py-1.5">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[13px]" style={{ color: '#CBD5E1' }}>Estimated Tax-Free Growth</span>
                    </div>
                    <span className="text-[13px] font-semibold text-white">
                      {fmtCAD(safe(results.taxFreeGrowth))}
                    </span>
                  </div>
                  <div className="flex items-center justify-between py-1.5">
                  ...
                  <div className="flex items-center justify-between py-1.5">
                    ...
                    <span
                      ...
                      style={{
                        color: results.overRoom ? '#f59e0b' : results.roomRemaining === 0 && results.availableRoom > 0 ? '#f59e0b' : results.availableRoom === 0 ? 'rgba(255,255,255,0.4)' : '#1DB584',
                      }}
                    >
                      {results.availableRoom === 0 ? '—' : results.overRoom ? `-${fmtCAD(results.overRoomBy)}` : fmtCAD(safe(results.roomRemaining))}
                    </span>
                  </div>
                </div>
                ...
                {(() => {
                  ...
                  const pillText =
                    roomPillState === 'over'
                      ...
                      : roomPillState === 'full'
                        ? 'Room fully used after first-year plan'
                        : roomPillState === 'remaining'
                          ? `Room remaining: ${fmtCAD(safe(results.roomRemaining))}`
                          : 'Enter available room above to check';
                  return (
                ...
                <div className="pt-1">
                  <p style={{ fontSize: '12.5px', color: 'rgba(255,255,255,0.52)', marginBottom: 8, fontStyle: 'italic' }}>
                    See your tax-free growth potential and contribution room analysis.
                  </p>
                  <button
                    className="btn-ai-cta-tfsa w-full font-bold overflow-hidden"
                    onClick={scrollToAI}
      ...
      {/* ══ Block C: Growth Breakdown + Room Usage ═══════════════════════════ */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 md:gap-6">

        {/* ── Left: TFSA Growth Composition (pill bar) ─────────────────── */}
        <div style={cardStyle}>
          <div className="p-5 md:p-6 h-full flex flex-col">
            <div className="mb-4 md:mb-5">
              ...
              <h3 className="text-base font-bold" style={{ color: '#0D1B2A' }}>TFSA Growth Composition</h3>
            </div>
            ...
            {results && (() => {
              ...
              const growth     = safe(results.taxFreeGrowth);
              ...
              const segments = [
                ...
                { label: 'Tax-Free Growth',     value: growth,  pct: growthPct,  color: '#1DB584', textColor: '#ffffff' },
              ];
              return (
                <div className="flex flex-col flex-1 gap-5">
                  ...
                  <div className="flex flex-col items-center text-center">
                    ...
                    <p style={{ color: '#94a3b8', fontSize: '11px', marginTop: 5, fontWeight: 500 }}>
                      after {results.yearsInvested} yr · tax-free growth illustration
                    </p>
                  </div>
                  ...
                  <div className="divide-y" style={{ borderColor: 'rgba(15,41,66,0.07)' }}>
                    ...
                    <div className="flex items-center justify-between py-1.5">
                      <span className="font-semibold text-slate-700" style={{ fontSize: '12.5px' }}>Projected TFSA Value</span>
                    ...
                    <div className="pt-1.5 pb-0.5">
                      <p className="text-[10.5px] leading-relaxed" style={{ color: '#94a3b8', fontStyle: 'italic' }}>
                        Tax-free growth illustration at {results.annualRate}% annual return. Actual returns vary.
                        Growth is not taxed while held in the TFSA.
                      </p>
                    </div>
                  </div>
        ...
        {/* ── Right: TFSA Room Usage (semi-circle gauge) ─────────────────── */}
        <div style={cardStyle}>
          <div className="p-5 md:p-6 h-full flex flex-col">
            <div className="mb-4 md:mb-5">
              <p className="text-[10px] font-bold uppercase tracking-[0.12em] mb-0.5" style={{ color: '#1DB584' }}>
              ...
              <h3 className="text-base font-bold" style={{ color: '#0D1B2A' }}>TFSA Room Usage</h3>
            </div>
            ...
            {!results && (
              <div className="flex flex-1 items-center justify-center py-10 rounded-xl"
                style={{ background: 'rgba(15,41,66,0.03)', border: '1px dashed rgba(15,41,66,0.10)' }}>
                <p className="text-sm text-center" style={{ color: 'rgba(13,27,42,0.3)' }}>
                  Enter details to see your room usage.
                </p>
              </div>
            )}
            ...
            {results && (() => {
              ...
              return (
                <div className="flex flex-col flex-1 gap-2">
                  ...
                  <div className="flex flex-col items-center">
                    {availableRoom === 0 ? (
                      <div className="flex flex-col items-center justify-center py-6 gap-2 rounded-xl w-full"
                        style={{ background: 'rgba(15,41,66,0.03)', border: '1px dashed rgba(15,41,66,0.10)' }}>
                        <p className="text-sm font-semibold text-center" style={{ color: '#0D1B2A' }}>Enter available room above</p>
                        <p className="text-xs text-center px-4" style={{ color: '#94a3b8' }}>Check CRA My Account or your financial institution for your exact TFSA contribution room.</p>
                      </div>
                    ) : (
                      <>
                  ...
                  {/* Room stat rows — condensed */}
                  <div className="divide-y" style={{ borderColor: 'rgba(15,41,66,0.07)' }}>
                    <div className="flex items-center justify-between py-1">
                      <span style={{ color: '#4B5563', fontSize: '12.5px' }}>Available Room (entered)</span>
                    ...
                    <div className="flex items-center justify-between py-1">
                      ...
                      <span className="font-semibold" style={{ color: overRoom ? '#ef4444' : roomRemaining === 0 && availableRoom > 0 ? '#f59e0b' : '#1DB584', fontSize: '12.5px' }}>
                        {availableRoom === 0 ? '—' : overRoom ? `-${fmtCAD(overRoomBy)}` : fmtCAD(safe(roomRemaining))}
                      </span>
                    </div>
                    <div className="pt-1 pb-0.5">
                      <p className="text-[10.5px] leading-relaxed" style={{ color: '#94a3b8', fontStyle: 'italic' }}>
                        First-year estimate only. Monthly contributions beyond year one may use future annual TFSA room which is not modeled here. Verify your exact room with CRA My Account before contributing.
                      </p>
                    </div>
                  </div>
      ...
      >
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <p className="text-white text-lg md:text-xl font-bold tracking-tight">
              FinCalc <span style={{ color: '#1DB584' }}>Smart</span> AI TFSA Analysis
            </p>
          </div>
          ...
          {!results && (
            <div className="flex flex-col items-center justify-center py-10 rounded-2xl gap-3"
              ...
              <p className="text-sm font-semibold" style={{ color: '#0D1B2A' }}>
                Enter your TFSA details above to see the analysis.
              </p>
              <p className="text-xs text-center px-4" style={{ color: '#94a3b8', maxWidth: 360 }}>
                The analysis will show your tax-free growth potential, contribution room usage, and first-year plan review.
              </p>
            </div>
          )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div className="rounded-2xl p-4 flex flex-col"
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-sm font-bold text-slate-800">TFSA Analysis</span>
                  </div>

                  <div className="flex-1 flex flex-col sm:flex-row">

                    {/* Gauge 1: Tax-Free Growth Score */}
                    <div className="flex-1 flex flex-col items-center text-center min-w-0 pb-3 sm:pb-0 sm:pr-3">
                      <div className="flex items-center w-full mb-2">
                        <p style={{ fontSize: '9px', fontWeight: 700, letterSpacing: '0.08em', color: '#94a3b8', textTransform: 'uppercase' }}>Tax-Free Growth</p>
                        <span className="ml-auto px-2 py-0.5 rounded-full shrink-0"
                    ...
                    {/* Gauge 2: Contribution Room Usage */}
                    {(() => {
                      ...
                      return (
                        <div className="flex-1 flex flex-col items-center text-center min-w-0 sm:pl-3">
                          ...
                          <p className="text-slate-500 mt-2" style={{ fontSize: '11px', lineHeight: 1.5 }}>
                            {!hasRoom
                              ? 'Enter your available TFSA contribution room above to see your first-year room usage.'
                              : overRoom
                                ? `Your first-year plan of ${fmtCAD(safe(results.plannedFirstYear))} exceeds your entered available room of ${fmtCAD(safe(availableRoom))} by ${fmtCAD(results.overRoomBy)}. Verify with CRA before contributing.`
                                : `Your first-year plan uses ${Math.round(roomUsedPct)}% of your entered available room of ${fmtCAD(safe(availableRoom))}.`
                            }
                          </p>
                        </div>
                      );
                  ...
                  {results.leverState === 'over-room' && (() => (
                    <>
                      <div className="flex flex-col gap-3 mt-1 lg:hidden">
                        ...
                        <div>
                          <p className="text-white font-semibold text-sm">exceeds your entered available room</p>
                          <p className="text-slate-400 text-xs mt-0.5">Verify your exact room with CRA My Account or your financial institution before contributing. This estimate is based on your entered room only.</p>
                        </div>
                        ...
                        <p className="text-[11px] italic" style={{ color: 'rgba(255,255,255,0.28)' }}>
                          This is not tax or legal advice. CRA is the authoritative source for your exact contribution room.
                        </p>
                      </div>
                      <div className="hidden lg:flex flex-col gap-3 mt-1 flex-1 justify-center">
                        ...
                        <div>
                          <p className="text-white font-semibold text-sm">exceeds your entered available room</p>
                          <p className="text-slate-400 text-xs mt-0.5">Verify with CRA My Account or your financial institution before contributing.</p>
                        </div>
                        <p className="text-[11px] italic" style={{ color: 'rgba(255,255,255,0.28)' }}>
                          This is not tax or legal advice. CRA is the authoritative source for your exact contribution room.
                        </p>
                    ...
                    <>
                      <div className="flex flex-col gap-3 mt-1 lg:hidden">
                        ...
                        <div>
                          ...
                          <p className="text-slate-400 text-xs mt-0.5">You may have additional capacity to contribute this year. Verify your exact room with CRA before contributing further.</p>
                        </div>
                        <div className="grid grid-cols-2 gap-2.5">
                          <div className="flex flex-col justify-center rounded-xl px-3 py-3"
                            style={{ background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.10)' }}>
                  ...
                  {results.leverState === 'room-nearly-used' && (() => (
                    <>
                      <div className="flex flex-col gap-3 mt-1 lg:hidden">
                        ...
                        <div>
                          <p className="text-white font-semibold text-sm">projected TFSA value after {results.yearsInvested} years</p>
                          <p className="text-slate-400 text-xs mt-0.5">your first-year plan uses most of your entered available room</p>
                        </div>
                        <div className="grid grid-cols-2 gap-2.5">
                          <div className="flex flex-col justify-center rounded-xl px-3 py-3"
                            style={{ background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.10)' }}>
                          ...
                          <div className="flex flex-col justify-center rounded-xl px-3 py-3"
                            style={{ background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.10)' }}>
                            <div className="text-white font-bold tabular-nums mb-1" style={{ fontSize: '1.05rem', lineHeight: 1.2 }}>{fmtCAD(safe(results.taxFreeGrowth))}</div>
                            <div className="flex items-center gap-1.5">
                              <TrendingUp className="w-3 h-3 shrink-0" style={{ color: '#1DB584' }} aria-hidden />
                              <span className="text-slate-400 text-xs">Tax-free growth</span>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div className="hidden lg:flex flex-col gap-3 mt-1 flex-1 justify-center">
                        <div className="flex items-stretch gap-3">
                          ...
                          <div className="flex flex-col justify-center rounded-xl px-4 py-7 shrink-0"
                            style={{ background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.10)', minWidth: 100 }}>
                            <div className="text-white font-bold tabular-nums mb-1.5" style={{ fontSize: '1.1rem', lineHeight: 1.2 }}>{fmtCAD(safe(results.taxFreeGrowth))}</div>
                            <div className="flex items-center gap-1.5">
                              <TrendingUp className="w-3 h-3 shrink-0" style={{ color: '#1DB584' }} aria-hidden />
                              <span className="text-slate-400 text-xs">Tax-free growth</span>
                            </div>
                        ...
                        <div>
                          <p className="text-white font-semibold text-sm">projected TFSA value — first-year plan uses most of entered room</p>
                  ...
                  {results.leverState === 'no-room' && (() => (
                    <div className="flex flex-col gap-3 mt-1 flex-1 justify-center items-center text-center px-2">
                      <div className="rounded-xl px-4 py-4 w-full"
                        style={{ background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.10)' }}>
                        <p className="text-white font-semibold text-sm mb-1">Enter your available contribution room</p>
                        <p className="text-slate-400 text-xs">Check CRA My Account or your financial institution to find your current TFSA contribution room, then enter it above to see the room analysis.</p>
                      </div>
                      <p className="text-[11px] italic" style={{ color: 'rgba(255,255,255,0.28)' }}>
                        The 2026 TFSA annual limit is $7,000. Your personal room depends on your contribution history, withdrawals, and years of eligibility.
                      </p>
                    </div>
                  ))()}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="rounded-2xl p-4" style={{ background: '#f0f9ff', border: '1px solid #bae6fd' }}>
                  ...
                  <p className="text-sm text-slate-700 leading-relaxed">
                    ...
                    <strong className="text-sky-700">{fmtCAD(safe(results.totalContributions))}</strong> before tax-free growth.
                    Future annual room beyond year one is not modeled — verify with CRA as each year passes.
                  </p>
                </div>

                {/* Card 2 — Tax-Free Growth Potential */}
                <div className="rounded-2xl p-4" style={{ background: '#f0fdf4', border: '1px solid #bbf7d0' }}>
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <p className="text-xs font-bold uppercase tracking-widest text-emerald-600">Tax-Free Growth Potential</p>
                  </div>
                  <p className="text-sm text-slate-700 leading-relaxed">
                  ...
                  return (
                    <div className="rounded-2xl p-4" style={{ background: bg, border: `1px solid ${bdr}` }}>
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <p className={`text-xs font-bold uppercase tracking-widest ${label}`}>Over-Contribution Risk</p>
                      </div>
                      <p className="text-sm text-slate-700 leading-relaxed">
                        {noRoom
                          ? 'Enter your available contribution room above to assess over-contribution risk. The 2026 TFSA annual limit is $7,000, but your personal room depends on your history with CRA.'
                          : isOver
                            ? <>Your first-year plan of <strong className="text-orange-700">{fmtCAD(safe(results.plannedFirstYear))}</strong> exceeds your entered available room of <strong className="text-orange-700">{fmtCAD(safe(results.availableRoom))}</strong>. If this reflects your actual CRA room, contributing this amount may result in an over-contribution. Verify your exact room with CRA My Account before proceeding. This calculator does not provide tax advice.</>
                            : <>Your first-year plan appears within your entered available room. TFSA amounts withdrawn are generally added back to contribution room on January 1 of the following year. The 2026 TFSA annual limit is $7,000, but your personal room may differ. Always verify with CRA My Account before contributing.</>
                        }
                      </p>
                    </div>
                  );
        ...
        <div className="flex items-start gap-2.5 px-5 md:px-6 py-4"
          ...
          <p className="text-slate-400 text-xs leading-relaxed">
            ...
            This analysis is for illustrative and informational purposes only. Projected values are estimates based on a fixed assumed annual return and user-entered contribution room. This tool does not verify or calculate exact CRA TFSA contribution room. Results do not account for taxes outside a TFSA, fees, inflation, market volatility, future annual TFSA limits, withdrawals, or recontribution timing. Over-contribution warnings are educational only and do not constitute tax, legal, or financial advice. Verify your exact room with CRA My Account or your financial institution before contributing.
          </p>
        </div>
      ...
      <p className="text-xs text-slate-400 text-center mt-4 pb-12 px-4 leading-relaxed max-w-4xl mx-auto">
        This calculator is for illustrative and informational purposes only. Projected values are estimates based on a fixed assumed annual return and user-entered contribution room. This tool does not verify or calculate exact CRA TFSA contribution room and does not account for taxes outside a TFSA, fees, inflation, market volatility, future annual TFSA limits, withdrawals, or recontribution timing. Users should verify their exact available contribution room with CRA My Account or their financial institution before making TFSA contributions. This does not constitute financial, investment, tax, or legal advice.
      </p>
