...
import {
  Activity, AlertTriangle, BookOpen, Car, CreditCard, Download,
...
function calcPayment(principal: number, annualRate: number, termMonths: number): number {
...
function computeResults(form: FormState, loanTermMonths: number): LoanResults | null {
  const vehiclePrice = parseFloat(form.vehiclePrice) || 0;
  const downPayment = Math.max(0, parseFloat(form.downPayment) || 0);
  ...
  const monthlyPayment = calcPayment(loanAmount, annualRate, loanTermMonths);
  ...
  const dtiPct = annualIncome > 0 ? (monthlyPayment / (annualIncome / 12)) * 100 : null;

  // Borrowing Cost Score — car loans typically 5–15% APR
  ...
  const statusLabel: LoanResults['statusLabel'] =
    ...
    : (annualRate > 12 || interestRatio > 25) ? 'Watch'
    : 'Healthy';
  ...
  const optNewPayment = calcPayment(optNewLoan, annualRate, loanTermMonths);
  ...
  // Shorter term opt
  const currentIdx = TERM_OPTIONS.indexOf(loanTermMonths as typeof TERM_OPTIONS[number]);
  ...
  if (shorterTermMonths !== null) {
    const sp = calcPayment(loanAmount, annualRate, shorterTermMonths);
  ...
  return {
    vehiclePrice, downPayment, downPaymentPct, loanAmount,
    annualRate, loanTermMonths,
    monthlyPayment, totalInterest, totalCost, interestRatio,
    dtiPct, loanScore, scoreLabel, statusLabel, statusColor, statusBg,
    isStrongEquity, downOptIncrease, downOptInterestSaved, downOptMonthlyDiff,
    shorterTermMonths, shorterTermSaved, shorterTermPaymentDiff, costPer1000,
  };
}
...
export default function CarLoanCalculator({
  ...
  const [loanTermMonths, setLoanTermMonths] = useState<number>(DEFAULT_TERM);
  ...
  const results = useMemo(() => computeResults(form, loanTermMonths), [form, loanTermMonths]);
  ...
  async function handleDownloadPDF() {
    ...
    try {
      ...
      const input = {
        ...
        vehiclePrice:           results.vehiclePrice,
        downPayment:            results.downPayment,
        downPaymentPct:         results.downPaymentPct,
        loanTermMonths:         results.loanTermMonths,
        isStrongEquity:         results.isStrongEquity,
        downOptIncrease:        results.downOptIncrease,
        downOptInterestSaved:   results.downOptInterestSaved,
        downOptMonthlyDiff:     results.downOptMonthlyDiff,
        shorterTermMonths:      results.shorterTermMonths,
        shorterTermSaved:       results.shorterTermSaved,
        shorterTermPaymentDiff: results.shorterTermPaymentDiff,
  ...
  return (
    <div className="flex flex-col gap-y-7 md:gap-y-8 min-w-0">
      ...
      <style>{`
        ...
        .btn-ai-cta-car {
          animation: teal-glow-car 2.8s ease-in-out infinite;
          transition: transform 180ms ease, box-shadow 180ms ease;
        }
        .btn-ai-cta-car:hover {
          transform: translateY(-2px);
          box-shadow: 0 0 0 1px rgba(29,181,132,0.75), 0 0 32px rgba(29,181,132,0.38) !important;
          animation: none;
        }
        .car-term-pills { display: flex; flex-wrap: wrap; gap: 6px; }
        .car-term-pill  { flex: 1; min-width: calc(33.33% - 5px); text-align: center; }
        @media (min-width: 480px) { .car-term-pill { min-width: 0; } }
      `}</style>
          ...
          <div className="p-5 md:p-6">
            ...
            <div className="flex items-center gap-3 mb-4 md:mb-5">
              <div style={{
                ...
                <Car size={15} color="white" strokeWidth={2.2} aria-hidden />
              </div>
              <div>
            ...
            <div className="grid grid-cols-2 gap-x-2 md:gap-x-5">
              ...
              <div className="space-y-3 min-w-0">
                ...
                <div>
                  <label className={navyLabelCls} style={navyLabelStyle}>
                    Vehicle Price
                    <Tooltip text="The full purchase price of the vehicle before taxes or fees. Enter the negotiated price you expect to pay the dealer." />
                  </label>
                  <NumericInput
                ...
                <div>
                  <label className={navyLabelCls} style={navyLabelStyle}>
                    ...
                    <Tooltip text="The APR on the loan. New car loans typically range 5–12%. Used car loans run higher, 8–18%. Check your lender or dealer offer for the exact rate." />
                  </label>
                  <NumericInput
              ...
              <div className="border-l pl-2 md:pl-5 space-y-3 min-w-0" style={{ borderColor: 'rgba(15,41,66,0.08)' }}>
                ...
                <div>
                  <label className={navyLabelCls} style={navyLabelStyle}>
                    Down Payment
                    <Tooltip text="The amount you pay upfront. A larger down payment reduces the loan amount, your monthly payment, and total interest paid." />
                  </label>
                  <NumericInput
                  ...
                  {(() => {
                    ...
                    return (
                      <p className="mt-0.5 text-[10px] font-semibold" style={{ color }}>
                        {pct.toFixed(1)}% of vehicle price
                      </p>
                    );
                  })()}
                </div>
                ...
                <div>
                  ...
                  <div className="car-term-pills">
                    {TERM_OPTIONS.map((mo) => (
                      <button
                        key={mo}
                        type="button"
                        className="car-term-pill font-semibold text-xs py-1.5 rounded-lg transition-colors duration-150"
                        onClick={() => setLoanTermMonths(mo)}
                        style={
                          loanTermMonths === mo
                ...
                <div>
                  <label className={navyLabelCls} style={navyLabelStyle}>
                    Annual Income
                    <Tooltip text="Optional. Used for affordability analysis — shows your monthly payment as a percentage of gross monthly income." />
                  </label>
                  <NumericInput
              ...
              <div className="flex flex-col flex-1 gap-4">
                ...
                >
                  ...
                  <p style={{ color: 'rgba(255,255,255,0.45)', fontSize: '12px', marginTop: 6, fontWeight: 500 }}>
                    {results.loanTermMonths} payments over {Math.round(results.loanTermMonths / 12 * 10) / 10} years
                  </p>
                </div>
                ...
                <div className="space-y-0 px-1 divide-y" style={{ borderColor: 'rgba(255,255,255,0.07)' }}>
                  ...
                  <div className="flex items-center justify-between py-1.5">
                    <span className="text-[13px] text-slate-300 font-medium">Total Loan Payments</span>
                    <span className="text-[13px] font-bold text-white">{fmt(safe(results.totalCost))}</span>
                  </div>
                  <div className="flex items-center justify-between py-1.5">
                    <span className="text-[13px]" style={{ color: '#CBD5E1' }}>Amount Financed</span>
                    <span className="text-[13px] font-semibold text-white">{fmt(safe(results.loanAmount))}</span>
                  </div>
                </div>
                ...
                <div className="pt-1">
                  ...
                  <button
                    className="btn-ai-cta-car w-full font-bold overflow-hidden"
                    onClick={scrollToAI}
                  ...
                  >
                    <span style={{
                      ...
                      Review Smart Car Loan Insights ↓
                    </span>
                    <span style={{
                      position: 'absolute', inset: 0,
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 md:gap-6">
        ...
        <div style={cardStyle}>
          <div className="p-5 md:p-6 h-full flex flex-col">
            ...
            {results && (() => {
              const allTerms = TERM_OPTIONS.map((mo) => {
                const payment = calcPayment(results.loanAmount, results.annualRate, mo);
                const totalCost = payment * mo;
                const totalInterest = Math.max(0, totalCost - results.loanAmount);
                return { months: mo, payment, totalCost, totalInterest, isCurrent: mo === results.loanTermMonths };
              });

              const cur = allTerms.find((t) => t.isCurrent) ?? allTerms[3];
                    ...
                    <div className="absolute top-0 bottom-0" style={{ left: YAXIS_W + 4, right: 0 }}>
                      ...
                      <div className="absolute left-0 right-0 flex gap-0.5" style={{ bottom: XLABEL_H, height: CHART_H }}>
                        {allTerms.map((term) => {
                          ...
                          return (
                            <div key={term.months} className="flex-1 flex justify-center items-end">
                                  ...
                                  <>
                                    ...
                                    <div style={{
                                      ...
                                      >
                                        ...
                                        <p style={{ fontSize: '13px', fontWeight: 800, color: '#F59E0B', lineHeight: 1 }}>{fmt(safe(term.totalInterest))}</p>
                      ...
                      <div className="absolute left-0 right-0 bottom-0 flex gap-0.5" style={{ height: XLABEL_H }}>
                        {allTerms.map((term) => (
                          <div key={term.months} className="flex-1 flex justify-center items-center">
                            {term.isCurrent ? (
                              <span
                              ...
                              <span style={{ fontSize: '8px', color: '#9BA8B5' }}>{term.months}m</span>
                            )}
                          </div>
                        ))}
                      </div>
      ...
      >
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <p className="text-white text-lg md:text-xl font-bold tracking-tight">
              FinCalc <span style={{ color: '#1DB584' }}>Smart</span> AI Car Loan Analysis
            </p>
          </div>
        ...
        <div className="p-4 md:p-5" style={{ background: '#f4f6f9' }}>
          ...
          {!results && (
            ...
            >
              ...
              <p className="text-sm font-semibold" style={{ color: '#0D1B2A' }}>
                Enter your car loan details above to get AI insights.
              </p>
              <p className="text-xs text-center px-4" style={{ color: '#94a3b8', maxWidth: 340 }}>
                The analysis will show your Borrowing Cost Score, rate competitiveness, and how your down payment affects total interest.
              </p>
            </div>
          )}
          ...
          {results && (
            <>
              {/* ── Row 1: white gauge card + navy smart lever card ── */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div className="flex-1 flex flex-col items-center text-center min-w-0 pb-3 sm:pb-0 sm:pr-3">
                      ...
                      <p className="text-slate-500 mt-2" style={{ fontSize: '11px', lineHeight: 1.5 }}>
                        {results.statusLabel === 'Healthy'
                          ? `${results.annualRate}% is a competitive car loan rate — borrowing cost is manageable.`
                          : results.statusLabel === 'Watch'
                            ? `${results.annualRate}% is above the typical new-car range. Shopping lenders may reduce your cost.`
                            : `At ${results.annualRate}%, interest cost is high. Compare 2–3 lenders before signing.`
                        }
                      </p>
                    </div>
                        ...
                        <div className="flex-1 flex flex-col items-center text-center min-w-0 pt-0 sm:pl-3">
                          ...
                          <div style={{ position: 'relative', width: 132, height: 132, flexShrink: 0 }}>
                            <svg viewBox="0 0 132 132" width="132" height="132" aria-hidden>
                              <circle cx="66" cy="66" r={GR} fill="none" stroke="rgba(15,41,66,0.09)" strokeWidth="10" strokeLinecap="round"
                                strokeDasharray={`${GARC.toFixed(1)} ${(GC - GARC).toFixed(1)}`} transform="rotate(150, 66, 66)" />
                              <circle cx="66" cy="66" r={GR} fill="none" stroke={affordColor} strokeWidth="10" strokeLinecap="round"
                                strokeDasharray={`${GFIL.toFixed(1)} ${(GC - GFIL).toFixed(1)}`} transform="rotate(150, 66, 66)"
                                style={{ transition: 'stroke-dasharray 0.9s ease' }} />
                            </svg>
                            <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', paddingBottom: 14 }}>
                          ...
                          <p className="text-slate-500 mt-2" style={{ fontSize: '11px', lineHeight: 1.5 }}>
                            {dti !== null
                              ? `Payment is ${Math.round(dti)}% of gross monthly income. ${dti <= 15 ? 'Well within comfortable range.' : dti <= 20 ? 'Manageable but leaves less buffer.' : 'Consider a larger down payment or longer term.'}`
                ...
                {/* Right — Down Payment Smart Lever */}
                <div
                ...
                >
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-sm font-bold" style={{ color: '#1DB584' }}>
                      {results.isStrongEquity ? 'Strong Down Payment' : 'Smart Optimization Found'}
                    </span>
                  </div>

                  {results.isStrongEquity ? (
                    /* Strong equity state */
                    <>
                      {/* Mobile */}
                      <div className="flex flex-col gap-3 mt-1 lg:hidden">
                        <div className="rounded-xl flex items-center justify-center px-4 py-4"
                          ...
                          <span className="font-extrabold tabular-nums"
                            ...
                            {results.downPaymentPct.toFixed(0)}%
                          </span>
                        </div>
                        <div>
                          <p className="text-white font-semibold text-sm">down payment — strong equity</p>
                          <p className="text-slate-400 text-xs mt-0.5">20%+ down reduces lender risk and total interest paid</p>
                        </div>
                        <div className="grid grid-cols-2 gap-2.5">
                          ...
                          <div className="flex flex-col justify-center rounded-xl px-3 py-3"
                            ...
                            <div className="flex items-center gap-1.5">
                              ...
                              <span className="text-slate-400 text-xs">Cost per {currencyPrefix}1k financed</span>
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="hidden lg:flex flex-col gap-3 mt-1 flex-1 justify-center">
                        ...
                        <div>
                          <p className="text-white font-semibold text-sm">down payment — strong equity position</p>
                          <p className="text-slate-400 text-xs mt-0.5">20%+ down reduces lender risk and minimizes total interest paid</p>
                        </div>
                      </div>
                    </>
                  ) : (
                    ...
                    <>
                      ...
                      <div className="flex flex-col gap-3 mt-1 lg:hidden">
                        <div className="rounded-xl flex items-center justify-center px-4 py-4"
                          ...
                          <span className="font-extrabold tabular-nums"
                            style={{ fontSize: 'clamp(1.9rem, 9vw, 2.4rem)', color: '#1DB584', letterSpacing: '-1.5px', lineHeight: 1 }}>
                            {fmt(safe(results.downOptInterestSaved))}
                          </span>
                        </div>
                        <div>
                          <p className="text-white font-semibold text-sm">estimated interest saved</p>
                          <p className="text-slate-400 text-xs mt-0.5">by increasing your down payment by {fmt(safe(results.downOptIncrease))}</p>
                        </div>
                        <div className="grid grid-cols-2 gap-2.5">
                          <div className="flex flex-col justify-center rounded-xl px-3 py-3"
                            style={{ background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.10)' }}>
                            <div className="text-white font-bold tabular-nums mb-1" style={{ fontSize: '1.05rem', lineHeight: 1.2 }}>
                              +{fmt(safe(results.downOptIncrease))}
                            </div>
                            <div className="flex items-center gap-1.5">
                              <Zap className="w-3 h-3 shrink-0" style={{ color: '#1DB584' }} aria-hidden />
                              <span className="text-slate-400 text-xs">Down payment increase</span>
                            </div>
                          </div>
                          <div className="flex flex-col justify-center rounded-xl px-3 py-3"
                            style={{ background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.10)' }}>
                            <div className="text-white font-bold tabular-nums mb-1" style={{ fontSize: '1.05rem', lineHeight: 1.2 }}>
                              -{fmtx(safe(results.downOptMonthlyDiff))}<span className="text-slate-400 font-normal text-xs">/mo</span>
                            </div>
                            <div className="flex items-center gap-1.5">
                              <CreditCard className="w-3 h-3 shrink-0" style={{ color: '#1DB584' }} aria-hidden />
                              <span className="text-slate-400 text-xs">Monthly reduction</span>
                            </div>
                          </div>
                        </div>
                      ...
                      <div className="hidden lg:flex flex-col gap-3 mt-1 flex-1 justify-center">
                        <div className="flex items-stretch gap-3">
                          ...
                          <div className="flex flex-col justify-center rounded-xl px-4 py-7 shrink-0"
                            ...
                            <div className="flex items-center gap-1.5">
                              <Zap className="w-3 h-3 shrink-0" style={{ color: '#1DB584' }} aria-hidden />
                              <span className="text-slate-400 text-xs">Down increase</span>
                            </div>
                          </div>
                          <div className="flex flex-col justify-center rounded-xl px-4 py-7 shrink-0"
                            style={{ background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.10)', minWidth: 100 }}>
                            <div className="text-white font-bold tabular-nums mb-1.5" style={{ fontSize: '1.1rem', lineHeight: 1.2 }}>
                              -{fmtx(safe(results.downOptMonthlyDiff))}<span className="text-slate-400 font-normal text-sm">/mo</span>
                            </div>
                            <div className="flex items-center gap-1.5">
                              <CreditCard className="w-3 h-3 shrink-0" style={{ color: '#1DB584' }} aria-hidden />
                              <span className="text-slate-400 text-xs">Monthly reduction</span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                {(() => {
                  ...
                  const positionText = isHigh
                    ? `Your rate of ${r}% is above the typical range. Comparing lenders or improving your credit score before applying could meaningfully reduce your cost.`
                    : isBorderline
                      ? `Your rate of ${r}% is in the mid-range. It may be worth checking one or two other lenders — even a 1% reduction saves money over the full term.`
                      : `Your rate of ${r}% is competitive for a car loan. You're well within the typical range for a well-qualified borrower.`;
                  return (
                ...
                <div className="rounded-2xl p-4" style={{ background: '#f0f9ff', border: '1px solid #bae6fd' }}>
                  ...
                  {results.shorterTermMonths ? (
                    <p className="text-sm text-slate-700 leading-relaxed">
                      Choosing a <strong className="text-sky-700">{results.shorterTermMonths}-month term</strong> costs{' '}
                      <strong className="text-sky-700">{fmtx(safe(results.shorterTermPaymentDiff))}/month</strong> more
                      but saves{' '}
                      <strong className="text-sky-700">{fmt(safe(results.shorterTermSaved))}</strong>{' '}
                      in total interest over the life of the loan.
                    </p>
                  ) : (
                    <p className="text-sm text-slate-700 leading-relaxed">
                      You&apos;ve selected the shortest available term (24 months) — interest cost is already minimised for this loan amount and rate.
                    </p>
                  )}
                </div>
                ...
                {(() => {
                  ...
                  const dtiCopy = !hasIncome
                    ...
                    : isHighDTI
                      ? `This payment is ${dti.toFixed(1)}% of your gross monthly income — above the 20% comfort guideline. A larger down payment or longer term may bring this into a comfortable range.`
                      : isMidDTI
                        ? `This payment is ${dti.toFixed(1)}% of your gross monthly income — slightly above the 15% guideline. Manageable, but factor in insurance and fuel costs alongside this payment.`
        ...
        >
          ...
          <p className="text-slate-400 text-xs leading-relaxed">
            ...
            Results assume a fixed interest rate and do not include taxes, dealer fees, insurance, or other charges.
      ...
      <p className="text-xs text-slate-400 text-center mt-4 pb-12 px-4 leading-relaxed max-w-4xl mx-auto">
        ...
        may not reflect actual auto loan terms, taxes, dealer fees, insurance, credit profile, approval
        requirements, or lender conditions. This does not constitute financial, tax, or legal advice.
