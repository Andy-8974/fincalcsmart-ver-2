...
> Last updated: 2026-07-22 — Sales Tax Calculator V1 validated and locked (source methodology reverse-engineered and independently confirmed for Add Tax / Remove Tax modes and the Canada 13-province/territory preset system; an independent Node re-derivation harness confirmed zero discrepancies across a 2,104-case reconciliation sweep — a 2,000-case cent-level dual-component sweep (GST+PST/QST across BC/MB/SK/QC, $0.01–$5.00) plus a 104-case exact-cent edge sweep across all 13 presets — confirming the last-component-absorbs-remainder pattern always sums exactly to total tax with no negative components; a display-only formatter bug fixed in "Total (tax-included)" (was using the 0-decimal `fmt()` instead of the 2-decimal `fmtx()`, e.g. showing "$115" instead of the correct "$114.98" for a Québec example); PDF/UI parity re-confirmed via a standalone transpiled-adapter harness with the adapter itself unmodified; responsive QA passed at 375/768/1280px with one minor cosmetic finding left documented not fixed (dual-component Tax Rate note text visually clips within its own badge at 375px for QC/BC/MB/SK — contained, not a page-level overflow); TypeScript and production-build checks both clean). See "Sales Tax Calculator V1 — Validation & Lock" below.
>
...
| Sales Tax Calculator V1 | ✅ LOCKED | `app/sales-tax-calculator/` | Dual-region; Add Tax / Remove Tax modes; Canada province/territory presets with GST/PST/QST/HST component breakdowns; USA manual rate only; no preset database for USA; Canada input order: Calculation Mode → Province/Territory → Amount → Tax Rate; Tax Breakdown donut; Add/Remove Tax Calculation card; AI Sales Tax Analysis (Tax Share card + dark smart result card + 3 insight cards); `Receipt` icon (purple #8B5CF6). **Validated & locked 2026-07-22** (see "Validation & Lock" record): source methodology and the 13-preset Add Tax / Remove Tax math independently re-derived and confirmed with zero discrepancies across a 2,104-case reconciliation sweep (2,000-case dual-component cent sweep + 104-case exact-cent edge sweep across all 13 presets); a display-only formatter bug fixed in "Total (tax-included)" (was rendering 0 decimals via `fmt()` instead of the correct 2-decimal `fmtx()`); PDF/UI parity re-confirmed, adapter unmodified — core tax math, preset rates, and component-breakdown logic unchanged. |
...
## Sales Tax Calculator V1 — Validation & Lock

> Validated: 2026-07-22 · Result: **PASS — V1 LOCK APPROVED**

Full end-to-end validation of the original V1 QA Record above (source methodology reverse-engineered and confirmed, independent financial validation via a standalone Node reimplementation of the Add Tax / Remove Tax and component-breakdown formulas, exhaustive boundary and cent-level reconciliation testing across all 13 Canada presets, PDF/UI parity review via a standalone transpiled-adapter harness, responsive QA, and technical QA), followed by a targeted display-only fix closing a formatter inconsistency identified during that validation. Core Add Tax / Remove Tax math, the 13 Canada province/territory preset rates, and the last-component-absorbs-remainder breakdown methodology were all confirmed unchanged — no financial or tax-rate logic was altered at any point in this round.

### Validated

- Source methodology reverse-engineered and validated against the calculator's own "How It Works" copy and `computeResults` / `computeComponentAmounts` logic.
- Add Tax math (`tax = amount × rate`, `total = amount + tax`) independently re-derived and verified with zero discrepancies across a boundary matrix (zero/near-zero amounts, decimal amounts, large amounts up to $999,999,999,999, zero/fractional/high rates, NaN/Infinity/negative adversarial input).
- Remove Tax math (`preTax = amount ÷ (1 + rate)`, `totalTax = amount − preTax`) independently re-derived and verified — reverse-reconstruction check confirmed re-adding tax to the derived pre-tax amount reproduces the original tax-included total to within $0.01 across every tested case.
- All 13 Canada province/territory presets verified against their published combined rates and components: AB/NT/NU/YT (5% GST only), BC/MB (12% = GST 5% + PST 7%), SK (11% = GST 5% + PST 6%), QC (14.975% = GST 5% + QST 9.975%), NB/NL/NS/PE (15%/15%/14%/15% HST) — alphabetical ordering and Manual Rate fallback confirmed.
- Cent-level component-breakdown reconciliation independently verified across a 2,104-case sweep: a 2,000-case sweep ($0.01–$5.00 in 1¢ steps across the 4 dual-component provinces BC/MB/SK/QC) plus a 104-case exact-cent edge sweep (8 representative amounts × all 13 presets) — in every case the last-component-absorbs-remainder pattern produced components summing exactly to total tax with no negative component amounts.
- "Tax share of total" wording confirmed distinct from and always ≤ the statutory rate in Remove Tax mode (mathematically expected, not a bug).
- UI/PDF parity validated via a standalone transpiled-adapter harness (`salesTaxAdapter.ts` compiled and run in isolation across Ontario, Québec dual-component, US Remove mode, boundary-rate, and zero-tax scenarios) — exact parity confirmed; the adapter itself was not modified.
- Responsive QA passed at 375px, 768px, and 1280px — no page-level horizontal overflow at any breakpoint.
- `npx tsc --noEmit` and `npx next build` both clean; `/sales-tax-calculator` unchanged at 8.28 kB / 116 kB First Load JS.

### Fix applied

**Display-only formatter correction** (`app/sales-tax-calculator/SalesTaxCalculator.tsx`) — the "Total (tax-included)" line in the Add/Remove Tax Calculation card was using the 0-decimal `fmt()` helper instead of the 2-decimal `fmtx()` helper used elsewhere in the same block, so a Québec example that should read "$114.98" was instead rounding to "$115". Changed the single line to call `fmtx(results.total)`. No calculation, state, or other formatted value was touched — this is a display-only fix; `results.total` itself was already correct to the cent.

### Confirmed working

- Live browser verification with a fresh (non-corrupted) dev server: Québec (QC) preset now correctly displays "$114.98" for Total (tax-included), matching the underlying `results.total` value exactly.
- Independent Node-harness regression (2,104/2,104 reconciliation cases + boundary matrix, 0 failures) re-deriving the Add Tax, Remove Tax, and component-breakdown formulas from scratch.
- Standalone transpiled-adapter PDF harness confirmed exact text/value parity with the live UI across Ontario, Québec (dual-component), US Remove mode, boundary-rate, and zero-tax scenarios.
- No console or dev-server errors observed at any point.
- `npx tsc --noEmit`: clean. `npx next build`: clean, all 43 routes compiled.

### Known non-blocking finding (not fixed, out of scope)

At 375px, the Tax Rate box's component-note text (e.g. "GST 5% + QST 9.975%") visually clips within its own badge for the four dual-component Canada provinces (QC/BC/MB/SK). This is a contained overflow within the badge, not a page-level layout break, and is consistent with similar minor mobile-legend-type cosmetic issues left documented-but-unfixed in prior locked calculators. Flagged for awareness only, not fixed.
